...
int main()
{
	...
	map<A, string, MyGreater> m2 = { {A(2,3),"2,3"},{A(-12,12),"-12,12"},{A(19,-9),"19,-9"} };		// function object class as Compare
	for (const auto& m : m2)
		cout << m.first << "\t" << m.second << endl;
	cout << endl;

	map<A, string, decltype(MyLess)*> m3(MyLess);		// 当比较操作是函数指针类型时，必须用具体的函数指针来初始化对象，这里用MyLess来初始化m3
	m3 = { {A(2,3),"2,3"},{A(-12,12),"-12,12"},{A(19,-9),"19,-9"} };
	for (const auto& m : m3)
		cout << m.first << "\t" << m.second << endl;
	cout << endl;

	using MyLessType = bool (*)(const A&, const A&);
	map<A, string, MyLessType> m4(MyLess);				// 当比较操作是函数指针类型时，必须用具体的函数指针来初始化对象，这里用MyLess来初始化m4
	m4.insert(pair<A, string>(A(-100, 100), "-100,100"));
	m4.insert(pair<A, string>(A(301, 67), "301,67"));
	auto ret = m4.insert(pair<A, string>(A(501, 67), "301,67"));		// 会插入失败！根据自定义的比较操作，关键字A(501, 67)和A(301, 67)相等，即关键字已存在，所以会插入失败
	if (!ret.second)
		cout << ret.first->first << " is already exited,map insert fail!" << endl;
	for (const auto& m : m4)
		cout << m.first << "\t" << m.second << endl;
	cout << endl;
