...
- 使用new和delete管理动态内存时常见的问题：
  ...
  - 同一块内存释放多次，会发生 double free 错误。通过在释放内存后将指针置为nullptr，可以部分程度上提供保护。因为当有多个指针指向相同的动态分配对象时，依然可能会发生 double free 错误。如果对其中一个指针进行了delete操作，对象的内存就被归还给自由空间了。如果我们随后又delete第二个指针，自由空间就可能被破坏，即发生 double free 错误。
