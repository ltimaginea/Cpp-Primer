...
void StrVec::reallocate(bool is_specified, size_t new_cap)
{
	...
	// 移动元素
	auto dest = std::uninitialized_copy(std::make_move_iterator(begin()), std::make_move_iterator(end()), new_data);
	free();
	elements_ = new_data;
	first_free_ = dest;
...
// Tips:
//	1. std::make_move_iterator 函数将一个普通迭代器转换为一个移动迭代器。此函数接受一个迭代器参数，返回一个移动迭代器。移动迭代器的解引用运算符生成一个右值引用。
